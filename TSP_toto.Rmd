...
while(success>0){
  ...
  for(len in 3:(n-1)){  # se fija una longitud
    ...
    for(path in 1:n){   # para cada longitud existen siempre n paths posibles por revisar en el ciclo inicial
      ...
      for(k in current_set){# vamos a buscar esta estructura en cada uno de los posibles caminos
        ...
        if(structure_length==structure1_length){ # si tienen longitudes iguales
          if(length(setdiff(structure, structure1))==0){ #y si tienen las mismas ciudades
            ...
            if(structure1_cost < structure_cost){ # si es mas barato es posible hacer un swap
              success <- success + 1 # considera que llevas un success
              structure_list[[success]] <- structure1 # agregalo a la lista
              structure_cost_vector[success] <- structure1_cost - structure_cost # agrega la utilidad 
              structure_index_list[[success]] <- c(k, index_begin, index_end) # informacion de donde empieza y donde acaba
            }
          }
        }
        if(structure_length==structure2_length){ # se hace lo análogo con el camino complemento
          if(length(setdiff(structure, structure2))==0){
            ...
            if(structure2_cost < structure_cost){
              success <- success + 1
              structure_list[[success]] <- structure2
              structure_cost_vector[success] <- structure2_cost - structure_cost
              structure_index_list[[success]] <- c(k, index_end, index_begin+n)
            }
          }
        }
      }
  ...
  if(success>0){
    index_min <- which.min(structure_cost_vector) # el numero de success que sera reemplazado
    ...
    index_head <- index_matrix[cycle_index, structure_replace[1]] # los indices head y tail del ciclo inicial
    index_tail <- index_matrix[cycle_index, tail(structure_replace, 1)] # que va a reemplazarse
    if(length(index_head:index_tail) == length(structure_replace)){
      ...
      index_cut <- which(structure_replace == cycle_index) #encuentra donde hay que hacer el corte de la estructura
      structure_head <- structure_replace[1:index_cut] # se parte en dos, head
      structure_tail <- structure_replace[-1:-(index_cut-1)] # y tail
      ...
      paths[cycle_index,] <- c(structure_tail, paths[cycle_index, index_begin:index_end], structure_head) # se reemplaza el camino
