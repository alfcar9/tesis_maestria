...
# Parameters
```{r}
n <- 75 # Number of Nodes in Plot
proportion_edges <- 1/2
greedy <- 3
...
for(num_iter in 1:m){
  for(i in 1:n){
     ...
     boolean_neighbors <- grado_vecinos_por_recorrer > greedy
...
while(exito>0){
  ...
  if(exito>0){
    ...
    costos_replace <- costos_ext[index_replace, index_begin:(index_end-1)]
    ...
    if(ini > end){
      ...
      costos_replace <- rev(costos_replace)
    }
    if(length(ini:end) == length(structure_replace)){
      caminos[ciclo_index,ini:end] <- structure_replace
      costos[ciclo_index, ini:(end-1)] <- costos_replace
    }
    else{
      edges_costs <- costos_replace
      index_cut <- which(structure == ciclo_index)
      ...
      caminos[ciclo_index,] <- c(structure_tail, caminos[ciclo_index,begin_index:end_index], structure_head)
      begin_index <- length(structure_head)
      costos[ciclo_index,] <- c(edges_costs_tail, costos[ciclo_index, begin_index:end_index], edges_costs_head)
      print("Warning")
    }
    for(i in 1:n){
      index_matrix[i,] <- sapply(1:n, function(j) index_function(i,j))
