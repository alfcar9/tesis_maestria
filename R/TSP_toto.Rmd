...
# Makes a matrix symmetric. If an entry is not symmetric takes the larger one. 
...
# Plots two solutions, with the edges that differ.
...
crossing_procedure <- function(city_current, city_min, num_iter, i){
  city21 <- city_current
  ...
  if(x22 - x21 != 0){ # Checks if slope is finite.
    m2 <- (y22 - y21)/(x22 - x21)
  }
  else{
    m2 <- Inf
  ...
  b2 <- y21-m2*x21
  for(k in (num_iter-1):1){ # ... recursive checking
    city11 <- paths[i,k]
    ...
    b1 <- y11 - m1 * x11
    if(m1 < Inf & m2 < Inf){ # both slopes are finite
      if(m1 != m2){ # not parallel
        x_intersect <- -(b2 - b1)/(m2 - m1) # the intersection is within the edge, not in the vertex
        ...
        if(b1 != b2){ # not overlapped means no intesection
          bool1 <- FALSE
          bool2 <- FALSE
        }
        else{ # lines overlapped, but does the segments have intersection?
          x11_aux <- min(x11, x12)
          x12_aux <- max(x11, x12)
          x21_aux <- min(x21, x22)
          x22_aux <- max(x21, x22)
    ...
    else if( m1 == Inf & m2 < Inf ){ # check for intersection
      x_intersect <- x11
    ...
    else{ # both are infinite, check if segments intersect
      if(x11 == x21){
        y11_aux <- min(y11, y12)
        y12_aux <- max(y11, y12)
        y21_aux <- min(y21, y22)
    ...
    if(bool1 & bool2){ # condition for finding crossing
      if(k != num_iter-1){
        ...
        num_iter_aux <- index_function(i, city11)
        if(num_iter_aux > 2){
          crossing_procedure(city11, city21, num_iter_aux, i) # recursive crossing procedure
        }
        city_current <- paths[i, num_iter] # update values for next iteration
        city21 <- city_current
        x21 <- cities_pos[city21,1]
        y21 <- cities_pos[city21,2]
      }
      else{ # specific case where the crossing is with the previous edge.
        replace_crossing <- rev(paths[i,k:num_iter])
        paths[i, k:num_iter] <<- replace_crossing
        dist1_replace <- cities_dist_original[paths[i, (num_iter-2)], city12]
        dist2_replace <- cities_dist_original[city11,city22]
        costs[i, c(num_iter-2,num_iter)] <<- c(dist1_replace, dist2_replace)
...
epsilon <- 1e-4
set.seed(11212)
proportion_edges <- 0.2  # Each node will only have this proportion of neighbors with respect the n neighbors
...
for(num_iter in 1:m){
  for(i in 1:n){
    ...
    if(num_iter>2){
      crossing_procedure(city_current, city_min, num_iter, i)
    } #end of if iter > 2
...
if(num_iter==(n-1)){
  ...
  for(i in 1:n){
    ...
    crossing_procedure(city_current, i, num_iter, i)
  }
} #end of if iter > 2
...
g1 <- TSP_plot(cities_pos_df, path_min_noswap, path_min_noswap) +  ggtitle("Greedy")  + labs (x = round(total_cost_min_noswap,2), y = "")
#g2 <- TSP_plot(cities_pos_df, paths[plot_index,], paths[plot_index,]) +  ggtitle("Greedy")  + labs (x = costs_cycles[plot_index], y = "")
g2 <- TSP_plot(cities_pos_df, path_min_swap, path_min_noswap) +  ggtitle("Swapping")  + labs (x = round(total_cost_min_swap,2), y = "")
