...
crossing_procedure <- function(city_current, city_min, num_iter, i){
  ...
  for(k in (num_iter-1):1){ # ... las restantes aristas
    ...
    if(m1 < Inf & m2 < Inf){
      ...
      y22_aux <- max(y21, y22)
      bool1 <- TRUE
      bool1 <- (y11_aux + epsilon < y21_aux & y21_aux < y12_aux - epsilon) | (y11_aux + epsilon < y22_aux & y22_aux < y12_aux - epsilon) | (y11_aux == y21_aux) | (y22_aux == y12_aux)
    }
...
#immediate_value <- 2 # When a node has this number of neighbors then it adds to the path regardless of the cost
immediate_value <- rep(c(2,3), ceiling(n/2))[1:n]
```
...
for(num_iter in 1:m){
  for(i in 1:n){
    ...
    immediate_neighbors <- degree_neighbors_to_go <= immediate_value[i]
...
cost_min_swap <- costs_cycles[cycle_index]
```
...
while(success>0){
  ...
  for(len in 3:(n-1)){  # se fija una longitud
    ...
    for(j in 1:n){   # para cada longitud existen siempre n paths posibles por revisar en el ciclo inicial
      ...
      for(k in current_set){# vamos a buscar este camino en cada uno de los n-1 caminos generados
