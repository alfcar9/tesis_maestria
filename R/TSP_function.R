...
crossing_procedure <- function(city_current, city_min, num_iter, i, paths_vec, costs_vec, cities_pos, cities_dist_original){
  ...
  b2 <- y21-m2*x21
  for(k in (num_iter-1):1){ # ... las restantes aristas
    city11 <- paths_vec[k]
    ...
    if(bool1 & bool2){ # condition for finding crossing
      if(k != num_iter-1){
        ...
        if(num_iter_aux > 2){
          list_crossing <- crossing_procedure(city11, city21, num_iter_aux, i, paths_vec, costs_vec, cities_pos, cities_dist_original)
